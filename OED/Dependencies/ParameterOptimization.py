...
class Convergence:
    ...
    def update_convergence(self,agents,it,threshold = -3):
        ...
        if min(SD_values) < self.lowest_SD:
            ...
            # print(self.position,self.position[-1],self.scores)
        ...
        if len(self.iteration) > 200:
            """no improvement over 100 generations = no movement"""
            if self.iteration[-1] - self.iteration[-2] > 200:
                terminate = True
            """terminate with no under threshold but with lower iteration"""  
            if self.lowest_SD < 0.1 and self.iteration[-1] - self.iteration[-2] > 100:
                terminate = True
        """terminate with 5% deviation"""
        if self.lowest_SD < 0.1 and self.iteration[-1] > 100:
            terminate = True
        ...
        for i in range(len(difference)):
                if difference[i] > 1.1 and it > 20 and it%5 == 0:
        ...
        if fixed != {}:
            ...
            for pID, value in fixed.items():
                for m in range(len(self.measurements)):
                    try:
                        self.measurements[i].model.fixed[pID] = value
                    except:
