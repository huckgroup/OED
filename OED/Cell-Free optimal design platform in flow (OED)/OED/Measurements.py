...
#this is a simle class which stores the information you are trying to extract from the model i.e. the parameters you are simulating and the scores you give 
...
def simulate_measurement(model,
    ...
    """store measurements"""      
    simdata           = {observable:data.simdata[observable] for observable in model.observables}
    measurement       = MeasurementObject(simdata,data.time,name = name, conditions = conditions,lumping = lumped,time_unit = 'min',flowrate = flowrate,time_dependent_parameters=time_dependent_parameters, simulation_time = time,fitfunc = fitfunc, store = store,show = show)
...
class MeasurementObject:
    def __init__(self,
                 ...
                 time_dependent_parameters = {},    
                 #which observables are lumped into a single observalbe
                 lumping         = {},
                 #the temperature, units and simulation time
                 temperature     = [],
                 units           = [],
                 simulation_time = [],
        ...
        """name of the dataset, reflected by experimental code"""
        self.name              = name
        ...
        """fit function of for the data presented"""
        ...
        """add potential flow rates"""
        if self.flowrate != 0:
            self.conditions["flow"] = self.flowrate
        ...
        self.profile = {}
#        print(self.name,'gfasdgasdgasdgadsgasdgadsgasdgasdgasdgadgasdga')
        ...
        """get start and end time of the measurement"""
        ...
        if fitfunc != 'leastsquaresfit':
            self.evaluation = {}
            criteria,binary = assessment[self.fitfunc](criteria = True)
            for state in self.observables:
                self.evaluation[state] = data_evaluation(self.profile[state],criteria)            
    ...
    def show(self,directory = '',name = '',scale = '',store = True,show = False):
        sns.set()
        ...
        if len(self.time_dependent_parameters) == 0:
            ...
            if self.temperature and self.flowrate:
                    textstr = '\n'.join(('Temperature: {}'.format(self.temperature,),
                                         'Flow rate: {}'.format(self.flowrate,),))
...
class GenerateExperiments:
        ...
        if len(conditions) < 10:
            ...
            for o in model.observables:
                ...
                for i in range(len(self.measurements)):
                    data[o].append(self.measurements[i].profile[o])
                    lsq[o].append(sum(self.measurements[i].profile[o]-mean))
    ...
    def return_random_pulse_measurement(self,
        ...
        measurement = simulate_measurement(self.model,name = name + "RandomPulse",dt = 1,coordinates = (coordinates,space),store = store)
