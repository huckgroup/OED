...
#               """set the parameters of the model from model variables"""
        ...
        if compilation:
            ...
            # ic = [0 + 1*10**random.choice([-12,-11,-10,-9])*random.random() for i in range(len(variables.ic))]
            ic = [i + 1 for i in variables.ic] 
...
#    
#class PythonDataObject:
#    def __init__(self,model,data,time,jacobian = []):
#        self.time  = time
#        self.rdata = data
#        """collect simdata by observable"""
#        self.simdata = {}
...
#        self.initial_state = self.rdata[-1] 
#        if jacobian:
#            self.jacobian = jacobian[-1]
#            
#        """extract the sensitivties"""
#        self.forward_sensitivities = {}
#        if len(model.states) < len(self.data[-1]):
#            for ID, index in model.map:
#                if type(ID) == tuple:
#                    self.forward_sensitivities[(ID)] = self.rdata[:,index]
#
#            
#    def sensitivity_update(self,measurement = {}):
#        """"calulate parameter probabilities with given the distance
#        so if the line fits don't mutate the paramter if does not mutate
#        you should eliminate quite a few structures in this manner"""
...
#            state,parameter = ID
#            if state in observables:
#                self.sensitivity_factor[(state,parameter)] = numpy.sum(numpy.transpose(lsq_factor[obs])*sensitivity_factor[(state,parameter)])
