...
#        for i in self.optimizations:
#            i.fittest.leastsquares(self.model,self.measurement)
#        self.lsq = [i.fittest.normalized_lsq for i in self.optimizations]
#        """the likelihood and the mean"""
#        self.likelihood_mean, self.likelihood_fit  = {},{}
#        """"plot the likelihoods as they can be extracted from the datasets, this is the extraction"""
#        for parameter,values in self.data.items():
#            index = {i:values[i] for i in range(len(values))}
#           
#            """sort the values by index"""
#            lsq_equivalent =  {i:[] for i in values} 
#            for i in range(len(values)):
#                lsq_equivalent[values[i]].append(self.lsq[i])
# 
#            """average and fittest rolling"""            
#            windowsize = 8
#            """store the fittest nad average (rolling) least squares error for a value"""
#            average,fittest = {},{}
#            """values sorted along the x axis low to high"""
#            sorted_values  = list(sorted(set(values)))
#
#            start = 0            
#            """get fittest values"""
#            unique_index = {i:sorted_values[i] for i in range(len(sorted_values))}
#            for i in range(len(sorted_values)):
#                if lsq_equivalent[unique_index[i]]:
#                    fittest[sorted_values[start]] = min(lsq_equivalent[unique_index[i]])
#                start += 1
#                
#            """index for parameter value start each window and take the average of the minima and the rolling average""" 
#            start = int(windowsize/2)
#            for each in window(range(len(unique_index)), windowsize):
#                window_lsq_min,window_lsq_average = [],[]
#                for i in each:
#                    window_lsq_min.append(min(lsq_equivalent[unique_index[i]]))
#                    window_lsq_average += lsq_equivalent[unique_index[i]]
#                    
#                """smallest window"""    
#                window_lsq_min = [i for i in window_lsq_min]
#                window_lsq_average = [i for i in window_lsq_average]
#                
#                """sort the fitest by window"""
#                fittest[sorted_values[start]] = sum(window_lsq_min)/float(len(window_lsq_min))
#                average[sorted_values[start]] = sum(window_lsq_average)/float(len(window_lsq_average))
#                start += 1      
#
#            """likelihood mean and fit for a parameter"""
#            self.likelihood_mean[parameter] = copy.copy(average)
#            self.likelihood_fit[parameter]  = copy.copy(fittest)

#        """plot this in a panel figure"""
#        colors = {i:numpy.random.rand(3,) for i in self.data.keys()}
#        """figure of fittest and rolling average likelihoods"""
#        figure = plt.figure(figsize=(15,15))
#        for parameter,v in self.likelihood_fit.items():
#            x, y = zip(*v.items())
#            x, y  = sort_axes(x,y)
#            plt.plot(x,y,color = colors[parameter])
#        plt.yscale('log')
#        plt.xscale('log')
#        plt.xlabel("Parameter Value",size = 14)
#        plt.ylabel("Lowest LSE Monte-Carlo Sampling",size = 14)
#        plt.legend(bbox_to_anchor=(1.05, 1),fancybox = True)
#        if store:
#            plt.savefig(self.directory + name + "_OPT_LSE.png",dpi = 600)     
#
#        
#        sns.set()
#        """plot the data and the least squares overview"""
#        figure = plt.figure(figsize=(10,10))
#        plt.subplot(2,1,1)
#        plt.plot(list(reversed(sorted(self.lsq))))
#        plt.xlabel("Optimization")
#        plt.ylabel("Normalized LSQ score")
#        if store:
#            plt.savefig(self.directory + name + "_OPT_NLSQ.png",dpi = 600)     
#        
#        """plot the filtered distributions the get an accurate assessment"""
#        plt.subplot(2,1,2)
#        fraction = int(0.8*len(self.lsq))
#        filtered_lsq = [list(reversed(sorted(self.lsq)))[i] for i in range(fraction)]
#        sns.distplot(filtered_lsq)
#        plt.xlabel("Optimization Distribution")
#        plt.ylabel("Normalized LSQ score")
#        plt.xlim([0,max(self.lsq)])
#        if store:
#            plt.savefig(self.directory + name + "_OPT_DLSQ.png",dpi = 600)                 
# 
        
#    
#    def leastsquares(self,model,measurement):
#        self.lsq = 0
#        for nmbr in range(len(measurement)):
#            for state in measurement[nmbr].observables:
#                 self.lsq += numpy.sum((numpy.array(self.simdata[nmbr][state]) - numpy.array(measurement[nmbr].profile[state])) ** 2)
# 
#        self.normalized_lsq = 0
#        for nmbr in range(len(measurement)):
#            for state in measurement[nmbr].observables:
#                 self.normalized_lsq += numpy.sum( ( ( numpy.array(self.simdata[nmbr][state]) - numpy.array(measurement[nmbr].profile[state]) ) / self.simdata[nmbr][state] ) ** 2 )
#            
#class agent_history:
#    def __init__(self,
#                 model,
...
#        self.simdata = data
#        
#        """"least squares both normalized and standard"""
#        self.lsq = 0
#        for nmbr in range(len(measurement)):
#            """"the observables of the measurement"""
#            for state in measurement[nmbr].observables:
